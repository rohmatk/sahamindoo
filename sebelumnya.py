...
total_row = pd.DataFrame({
    'Bulan': ['TOTAL'],
    'Total Lokal': [df_summary['Total Lokal'].sum()],
    'Total Asing': [df_summary['Total Asing'].sum()],
    'Total': [df_summary['Total'].sum()]
})

# Gabungkan ke bawah
df_summary_total = pd.concat([df_summary, total_row], ignore_index=True)

st.subheader("📄 Data Ringkasan Bulanan (dengan TOTAL)")
st.dataframe(df_summary_total, use_container_width=True)
...
#Bagian Berita Harian

# Membaca file kode saham dari CSV
try:
    df_alias = pd.read_csv("data/kode_saham/kode_saham.csv")

    # Buat mapping alias kode → nama (hapus "Tbk." dan spasi ekstra)
    saham_alias = {
    ...
    st.warning(f"⚠️ Gagal membaca file alias saham: {e}")
    saham_alias = {}

# Buat kata kunci pencarian berita
keyword_cari = f"{selected_code} {saham_alias.get(selected_code, '')}".strip()
st.caption(f"📌 Kata kunci pencarian berita: `{keyword_cari}`")

# Ambil berita berdasarkan kombinasi kode + nama perusahaan
berita = ambil_berita_google(keyword_cari)

#experiment
st.subheader("🧪 Semua Berita (Mentah)")
for item in berita:
    st.markdown(f"- `{item['pubDate']}` | {item['judul']}") 
    if 'description' in item:
        st.markdown(item['description'], unsafe_allow_html=True)

nama_alias = saham_alias.get(selected_code, '')
keyword_cari = f'"{selected_code}" OR "{nama_alias}" saham'
st.subheader(keyword_cari)
